...
async def _get_controller() -> AuthController:
    return AuthController(AuthService(await get_admin_client(), await get_anon_client()))
...
async def register(payload: RegisterRequest, controller: AuthController = Depends(_get_controller)):
    return await controller.register(payload)

@router.post("/login",response_model=AuthResponse,status_code=status.HTTP_200_OK)
async def login(payload:LoginRequest,controller: AuthController = Depends(_get_controller)):
